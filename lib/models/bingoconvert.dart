...
List<BingoSala> bingoSalaFromMap(String str) =>
    List<BingoSala>.from(json.decode(str).map((x) => BingoSala.fromMap(x)));

String bingoSalaToMap(List<BingoSala> data) =>
...
class BingoSala {
  ...
  factory BingoSala.fromMap(Map<String, dynamic> json) => BingoSala(
        ...
        premios:
            List<Premio>.from(json["premios"].map((x) => Premio.fromMap(x))),
      );
